...
def sift(alist, low, high):
    ...
    tmp = alist[i] # 保存堆顶
    while j <= high: # 越界判断
        if j+1 <= high and alist[j+1] > alist[j]: # 右子树比较大,则指向右子树
            j = j+1
...
def heap_sort(alist):
    # 1、建堆
    # 先找到最后一个不是叶子节点的根节点，为(n-2)//2 (若叶子节点为i，则他的父节点为(i-1)//2，然后 i=n-1)
    # 再向上循环根节点，从小到大
    n = len(alist)
    ...
    # 2、挨个出数，按升序排列，每轮一个，所以需要n轮
    for i in range(n-1, -1, -1):
        # 交换首位，节省空间
        alist[0], alist[i] = alist[i], alist[0]
        # 最后一个元素为-1，因为尾部此时是最大元素，不参与调整
        sift(alist, 0, i-1)
