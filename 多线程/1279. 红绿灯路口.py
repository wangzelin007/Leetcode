# 这是两条路的交叉路口。
# 第一条路是 A 路，车辆可沿1 号方向由北向南行驶，也可沿2 号方向由南向北行驶。
# 第二条路是 B 路，车辆可沿3 号方向由西向东行驶，也可沿 4 号方向由东向西行驶。
# 每条路在路口前都有一个红绿灯。红绿灯可以亮起红灯或绿灯。
# 绿灯表示两个方向的车辆都可通过路口。
# 红灯表示两个方向的车辆都不可以通过路口，必须等待绿灯亮起。
# 两条路上的红绿灯不可以同时为绿灯。
# 这意味着，当 A 路上的绿灯亮起时，B 路上的红灯会亮起；
# 当 B路上的绿灯亮起时，A路上的红灯会亮起.
# 开始时，A 路上的绿灯亮起，B 路上的红灯亮起。
# 当一条路上的绿灯亮起时，所有车辆都可以从任意两个方向通过路口，直到另一条路上的绿灯亮起。
# 不同路上的车辆不可以同时通过路口。
# 给这个路口设计一个没有死锁的红绿灯控制系统。
# 实现函数void carArrived(carId, roadId, direction, turnGreen, crossCar):
# carId为到达车辆的编号。
# roadId为车辆所在道路的编号。
# direction为车辆的行进方向。
# turnGreen是一个函数，调用此函数会使当前道路上的绿灯亮起。
# crossCar是一个函数，调用此函数会允许车辆通过路口。
# 当你的答案避免了车辆在路口出现死锁，此答案会被认定为正确的。
# 当路口已经亮起绿灯时仍打开绿灯，此答案会被认定为错误的。
# 示例 1:
...
# ]
# 解释: 这是一个无死锁的方案。
# 注意，在 A 路上的绿灯亮起、5 号车通过前让 4 号车通过，也是一个正确且可被接受的方案。
# 提示：
# 1 <= cars.length <= 20
